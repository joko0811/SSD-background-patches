...
def calc_class_TP(
    detections, ground_truthes, total_class, iou_scores, iou_threshold=0.5
):
    ...
    aggregation_TP_by_class = torch.zeros(
        total_class, device=tp_flag.device, dtype=torch.int64
    ).scatter_add(0, detections.class_labels, tp_flag.to(torch.int64))
...
def calc_class_FP(
    ...
    aggregation_FP_by_class = torch.zeros(
        total_class, device=fp_flag.device, dtype=torch.int64
    ).scatter_add(0, detections.class_labels, fp_flag.to(torch.int64))
...
def calc_class_FN(ground_truthes, total_class, iou_scores, iou_threshold=0.5):
    ...
    aggregation_FN_by_class = torch.zeros(
        total_class, device=fn_flag.device, dtype=torch.int64
    ).scatter_add(0, ground_truthes.class_labels, fn_flag.to(torch.int64))
