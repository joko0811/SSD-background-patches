...
def train_adversarial_image(image, mask_image, config: DictConfig, tbx_writer=None):
    ...
    with torch.no_grad():
        ...
        encoded_tuple = s3fd_util.image_encode(
            image)
        s3fd_image = encoded_tuple[0].to(
            device=device, dtype=torch.float)
        scale = encoded_tuple[1].to(
            device=device, dtype=torch.float)

        s3fd_mask_image = s3fd_util.image_encode(
            mask_image, is_mask=True)[0].to(device=device, dtype=torch.float)
        ...
        gt_detections.set_group_info(torch.from_numpy(
    ...
    for perturbate_iter in tqdm(range(max_epoch), leave=(tbx_writer is not None)):
        ...
        if detections is None:
            ...
            pil_adv_image = s3fd_util.image_decode(
                s3fd_adv_image[0], scale[0])
            tbx_writer.add_image(
                "adversarial_image", transforms.functional.to_tensor(pil_adv_image), perturbate_iter)
            break

        tpc_loss, tps_loss, fpc_loss, end_flag = total_loss(
            detections, gt_detections, background_patch_boxes, s3fd_adv_background_image.shape[-2:], config.loss)
        ...
        with torch.no_grad():
            ...
            if perturbate_iter == 0:
                ...
                background_patch_boxes = pf.initial_background_patches(
                    gt_detections, gradient_image, config.initial_background_patches, scale=bg_scale)
            else:
                # パッチ領域を拡大する（縮小はしない）
                background_patch_boxes = pf.expanded_background_patches(
                    background_patch_boxes, gt_detections, gradient_image, config.expanded_background_patches, scale=bg_scale)
            ...
            if tbx_writer is not None:
                ...
                bp_image = imgdraw.draw_boxes(
                    s3fd_util.image_decode(s3fd_adv_background_image[0]), background_patch_boxes*bg_scale)
                tbx_writer.add_image(
                    "background_patch_boxes", transforms.functional.to_tensor(bp_image), perturbate_iter)
                ...
                if detections is not None:
                    ...
                    tbx_writer.add_image(
                        "adversarial_image", transforms.functional.to_tensor(det_image), perturbate_iter)

    adv_background_image = s3fd_util.image_decode(s3fd_adv_background_image[0])
    return adv_background_image


@hydra.main(version_base=None, config_path="../conf/", config_name="train_background_patch")
        ...
        case "monitor":
            ...
            with torch.autograd.detect_anomaly():
                adv_background_image = train_adversarial_image(
                    image, mask_image, config.train_adversarial_image, tbx_writer=tbx_writer)
