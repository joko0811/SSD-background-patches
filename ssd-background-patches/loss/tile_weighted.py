...
def total_loss(detections: DetectionsBase, ground_truthes: DetectionsBase, config: DictConfig):
...
def tpc_loss(detections: DetectionsBase, det_weight):
    score = (det_weight*torch.log((1-detections.conf)+1e-5)).sum() / \
        len(detections)
    return score


def fpc_loss(detections: DetectionsBase, det_weight):
    score = ((1-det_weight)*torch.log(detections.conf+1e-5)).sum() / \
        len(detections)
    return score
